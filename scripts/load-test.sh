...
WALLET_ACCOUNT=acc_usr_load_user_wallet
...
echo "==> seeding one user, one merchant, three ledger accounts"
psql "$DSN" -q <<SQL >/dev/null
INSERT INTO users (id, phone, full_name, pin_hash, kyc_tier, status)
  VALUES ('${USER_ID}', '08120000000', 'Load Test', 'x', 'verified', 'active');
...
INSERT INTO accounts (id, owner_type, owner_id, account_type) VALUES
  ('${WALLET_ACCOUNT}', 'user', '${USER_ID}', 'user_wallet'),
...
redis-cli -p "$REDIS_PORT" SET "marspay:balance:${WALLET_ACCOUNT}" 100000000000000 >/dev/null
